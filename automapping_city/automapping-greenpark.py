...
outmap_fd = os.path.join(root_path, "output_map02")  ##create outmap folder
outmxd_fd = os.path.join(root_path, "output_mxd02")  ##create outmxd folder
mikfolder(outmap_fd)
mikfolder(outmxd_fd)
indi_gdb_path = os.path.join(os.path.join(root_path, "data02_gdb"), "data02.gdb") #手动添加
...
resi_gdb = raw + r'02data\resi_aoi.gdb' ##原始路径+居住小区
park_gdb = raw + r'02data\greenpark.gdb' ##原始路径+绿地
park_buffer_gdb = raw + r'02data\greenpark_buffer.gdb' ##原始路径+绿地覆盖
...
for shp_stat in shp_list[1:]:
    ...
    addtown('居住小区', resi_gdb, shp_stat, name_element, zoomYN='no')
    addtown('公园绿地', park_gdb, shp_stat, name_element, zoomYN='no')
    addtown('公园绿地服务范围', park_buffer_gdb, shp_stat, name_element, zoomYN='no')
    ...
    m.removeLayer(m.listLayers("{}".format(name_element + '居住小区'))[0])
    m.removeLayer(m.listLayers("{}".format(name_element + '公园绿地'))[0])
    m.removeLayer(m.listLayers("{}".format(name_element + '公园绿地服务范围'))[0])

...
outmap_fd = os.path.join(root_path, "output_map05")  ##create outmap folder
outmxd_fd = os.path.join(root_path, "output_mxd05")  ##create outmxd folder
mikfolder(outmap_fd)
mikfolder(outmxd_fd)
indi_gdb_path = os.path.join(os.path.join(root_path, "data05_gdb"), "data05.gdb") #手动添加
...
for shp_stat in shp_list:
    ...
    addtown('避难场所', bncs_gdb, shp_stat, name_element, zoomYN='no')
    ...
    m.removeLayer(m.listLayers("{}".format(name_element + '避难场所'))[0])

...
outmap_fd = os.path.join(root_path, "output_map06")  ##create outmap folder
outmxd_fd = os.path.join(root_path, "output_mxd06")  ##create outmxd folder
mikfolder(outmap_fd)
mikfolder(outmxd_fd)
indi_gdb_path = os.path.join(os.path.join(root_path, "data06_gdb"), "data06.gdb") #手动添加
...
for shp_stat in shp_list:
    ...
    addtown('二级以上医院', hospital_gdb, shp_stat, name_element, zoomYN='no')
    addtown('二级以上医院覆盖范围', hospital_buffer_gdb, shp_stat, name_element, zoomYN='no')
    ...
    m.removeLayer(m.listLayers("{}".format(name_element + '二级以上医院'))[0])
    m.removeLayer(m.listLayers("{}".format(name_element + '二级以上医院覆盖范围'))[0])

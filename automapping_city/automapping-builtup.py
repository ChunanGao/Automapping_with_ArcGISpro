...
outmap_fd = os.path.join(root_path, "output_map01")  ##create outmap folder
outmxd_fd = os.path.join(root_path, "output_mxd01")  ##create outmxd folder
mikfolder(outmap_fd)
mikfolder(outmxd_fd)
indi_gdb_path = os.path.join(os.path.join(root_path, "data01_gdb"), "data01.gdb") #手动添加
...
for shp_stat in shp_list[1:]:
    ...
    # 添加变量图层
    addtown('市辖区边界', shixiaqu_gdb, shp_stat, name_element, zoomYN='yes')
    addtown('建成区边界', builtup_gdb, shp_stat, name_element, zoomYN='no')

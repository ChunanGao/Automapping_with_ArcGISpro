...
outmap_fd = os.path.join(root_path, "output_map07")  ##create outmap folder
outmxd_fd = os.path.join(root_path, "output_mxd07")  ##create outmxd folder
mikfolder(outmap_fd)
mikfolder(outmxd_fd)
indi_gdb_path = os.path.join(os.path.join(root_path, "data07_gdb"), "data07.gdb") #手动添加
...
for shp_stat in shp_list:
    ...
    addtown('消防站点位', firestation_gdb, shp_stat, name_element, zoomYN='no')
    addtown('消防站服务范围', firestation_service_gdb, shp_stat, name_element, zoomYN='no')
    ...
    m.removeLayer(m.listLayers("{}".format(name_element + '消防站点位'))[0])
    m.removeLayer(m.listLayers("{}".format(name_element + '消防站服务范围'))[0])
